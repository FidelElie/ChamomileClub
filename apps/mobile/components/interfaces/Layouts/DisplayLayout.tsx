...
export const DisplayLayout = (props: DisplayLayoutProps) => {
	...
	return (
		<Flex className="bg-green h-full pt-5 px-6" safe={safe}>
			<Flex.Row className={twJoin("items-center", onBack ? "justify-between" : "justify-end")}>
				{
					onBack && (
						<Button className="border border-white rounded-full p-2.5" onPressIn={onBack}>
							<AntDesign name="left" color={COLORS["yellow"]} size={15}/>
						</Button>
					)
				}
				<Flex.Column className="items-end">
					<Heading size="xl" numberOfLines={1}>{ title }</Heading>
					<Copy size="sm" color="yellow">{ subtitle }</Copy>
				</Flex.Column>
			</Flex.Row>

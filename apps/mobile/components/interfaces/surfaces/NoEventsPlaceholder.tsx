...
import { Show, Flex, Heading, Copy, Button, COLORS } from "@thechamomileclub/ui";
...
export const NoEventsPlaceholder = (props: NoEventsPlaceholderProps) => {
	...
	return (
		<Show if={!user?.roles.includes("PROSPECT")} else={<Copy>Hello World</Copy>}>
			<Button
				className="w-full px-0"
				onPressIn={() => navigation.navigate("CreateEvent")}
			>
				<Flex
					className="h-40 w-full bg-midnight rounded-lg px-4 py-2.5 justify-end"
				>
					<Flex.Row className="justify-between items-end">
						<Flex.Column>
							<Heading size="base">No events yet</Heading>
							<Copy size="xs" color="yellow">Create One?</Copy>
						</Flex.Column>

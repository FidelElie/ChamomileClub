...
import { useQueryClient, type UserSchema } from "@thechamomileclub/api";
...
export const OnboardingFlow = (props: { user: UserSchema }) => {
	const { user } = props;
	...
	const editFields = (data: Partial<typeof fields>) => setFields(currentFields => ({
		...currentFields, ...data
	}));
		...
		<Flex className="w-full space-y-8">
			<Show if={stage === "INTRODUCTION"}>
				<Flex className="flex-grow space-y-3">
					<Copy color="cream" className="text-center">{user.description}</Copy>
				</Flex>
			</Show>
			...
			<Flex>
				<Button.Primary onPressIn={handleStageSubmission}>
					<Copy>{ stage === "INTRODUCTION" ? "Get started" : "Confirm Membership" }</Copy>
				</Button.Primary>
				<Show if={stage === "DETAILS"}>
					<Button.Secondary onPressIn={() => setStage("INTRODUCTION")} className="mt-3">

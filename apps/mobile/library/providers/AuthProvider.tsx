...
export const AuthProvider = ({ children, suspend }: AuthProviderProps) => {
	const [token, setToken] = useAsyncStorage(AppConfig.SESSION_STORAGE_KEY, initialContext.token);
	...
	const logout = () => { logoutUser.mutate(); }

	useEffect(() => { currentUserQuery.refetch(); }, [token, initialising]);
...
type AuthContextType = {
	...
	setInitialising: (state: boolean |  ((currentState: boolean) => boolean)) => void
}

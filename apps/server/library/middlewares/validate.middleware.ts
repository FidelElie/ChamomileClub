...
export const validateRequestEntities = (entities: RequestEntities) => {
  ...
  return (req: ApiRequest, res: NextApiResponse, next: NextHandler) => {
    ...
    const body = validateBody
      ? validateBody.safeParse(contextBody)
      : passThrough;

    const query = validateQuery
      ? validateQuery.safeParse(contextQuery)
      : passThrough;

    const params = validateParams
      ? validateParams.safeParse(contextParams)
      : passThrough;

    const headers = validateHeaders
      ? validateHeaders.safeParse(contextHeaders)
      : passThrough;
    ...
    if (body.data) {
      req.body = { ...(contextBody || {}), ...body.data };
    }

    if (query.data) {
      req.query = { ...contextQuery, ...query.data };
    }

    if (params.data) {
      req.params = { ...contextParams, ...params.data };
    }

    if (headers.data) {
      req.headers = { ...contextHeaders, ...headers.data };
    }

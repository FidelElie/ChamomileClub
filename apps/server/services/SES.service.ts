...
export const SESService = (client: SESClient, serviceConfig: ServiceConfig) => {
	...
	const send = async (config: BaseSendConfig<EmailTemplate>) => {
		...
		try {
			const emailCommand = new SendEmailCommand({ Source, Destination, Message, Tags });
	...
	const parseEmailDestination = (config: BaseSendConfig<EmailTemplate>) => {
		const parseSignatures = (signatures: EmailSignature[] | EmailSignature) => (
			...
			signature => parseEmailSignature(signature)
		);
	...
	const parseEmailMessage = (config: BaseSendConfig<EmailTemplate>) => {
		...
		const Data = typeof content === "function" ? parseEmailTemplate(content, model) : content;
	...
	const parseEmailTemplate = (element: EmailTemplate, props?: ComponentProps<EmailTemplate>) => {
		return renderToStaticMarkup(createElement(element, props));
	}

	const parseEmailSignature = (recipient: string | { name: string, email: string }) => {
	...
	return {
		sendMagicLink: (config: TemplatedEmailConfig<typeof MagicLinkEmail>) => send({
			...config,
			id: "magic-link",
			subject: "Login to your account",
			content: MagicLinkEmail
		}),
		sendMemberInvitation: (config: TemplatedEmailConfig<typeof MemberInvitationEmail>) => send({
			...config,

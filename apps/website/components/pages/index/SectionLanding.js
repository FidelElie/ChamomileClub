...
const LandingSection = (props) => {
  ...
  const { mediaRef, mediaLoaded, mediaLoading, mediaError } = useMedia(
    "./videos/landing-video.mp4",
  );
  ...
  return (
    <div className="w-full h-screen flex flex-col box-border relative">
        ...
        {!mediaError && (
          <video
            ...
            className={joinClasses(
              "w-full h-full object-cover transition-opacity absolute top-0 left-0 duration-300",
              {
      ...
      <div
        className={joinClasses(
          "w-full h-full flex-col flex justify-center items-center bg-black bg-opacity-50 z-10 transition-opacity duration-300 delay-300 lg:flex-col",
          {
...
const VideoLoading = ({ mediaLoading }) => (
  <div
    className={joinClasses("w-full h-full flex items-center justify-center", {
      "opacity-0": !mediaLoading,
      "opacity-100": mediaLoading,
    })}
  >

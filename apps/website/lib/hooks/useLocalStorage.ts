...
const useLocalStorage = <T = JSONParsable>(
  key: string,
  initialValue: JSONParsable = null,
) => {
  const [storageValue, setStorageValue] = useState<JSONParsable>();

  const editStorageValue = (value: T) => {
    setStorageValue(value as JSONParsable);
  };
  ...
  useEffect(() => {
    if (localStorage) {
      ...
      setStorageValue(
        savedStorageValue ? JSON.parse(savedStorageValue) : initialValue,
      );
    }
  }, [initialValue, key]);

...
const determineFolderPaths = (config: ProgramConfig, rootDir: string) => {
	...
	if (!Object.keys(config).length) {
		...
		return [
			...
			all || cache ? findFolders(rootDir, ".turbo").concat(findFolders(rootDir, ".expo")) : [],
			all || node ? findFolders(rootDir, "node_modules") : []
		].flat();
	}
}

const findFolders = (dirPath: string, identifier: FolderIdentifiers): string[] => {
	if (!fs.lstatSync(dirPath).isDirectory()) { throw new Error("Valid directory path required"); }

	const contents = fs.readdirSync(dirPath);

	const directories = contents.filter(
		content => fs.lstatSync(path.join(dirPath, content)).isDirectory() && content !== "node_modules"
	);

	const modulesPath = contents.includes(identifier) ? [path.join(dirPath, identifier)] : [];

	const nodeModulesInSubDirectories = directories.map(
		dirName => findFolders(path.join(dirPath, dirName), identifier)
	);
